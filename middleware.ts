...
const token = Deno.env.get('NEXT_PUBLIC_MAPBOX_ACCESS_TOKEN');
...
export async function middleware(nextRequest: NextRequest) {
  ...
  // use the Mapbox API to load lat/lng data
  const location = await getLocationDetails(place, token);
  ...
  response.setPageProp('location', location);
